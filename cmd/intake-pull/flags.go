...
func parseFlags() *Flags {
	flags := &Flags{
		IntakePullOpts: &qtr.IntakePullOpts{},
	}

	var dsn string
	flag.StringVar(&flags.IntakePullOpts.QueueName, "queue", os.Getenv("QTR_INTAKE_QUEUE"), "intake queue name. use $QTR_INTAKE_QUEUE env")
	...
	flag.IntVar(&flags.NAgents, "nagents", cliutil.GetIntEnv("QTR_INTAKE_PULL_NAGENTS", 1), "number of agents. use $QTR_INTAKE_PULL_NAGENTS env")
	flag.DurationVar(&flags.Interval, "interval", cliutil.GetDurEnv("QTR_INTAKE_PULL_INTERVAL", 1*time.Second), "poll interval. use $QTR_INTAKE_PULL_INTERVAL env")
	flag.DurationVar(&flags.ErrInterval, "err-interval", cliutil.GetDurEnv("QTR_INTAKE_PULL_ERR_INTERVAL", 1*time.Minute), "error wait interval. use $QTR_INTAKE_PULL_ERR_INTERVAL env")
	flag.IntVar(&flags.MaxRecvNum, "max-recv", cliutil.GetIntEnv("QTR_INTAKE_PULL_MAX_RECV", 1), "maximum number of received messages. use $QTR_INTAKE_PULL_MAX_RECV env")
	...
	if flags.IntakePullOpts.QueueName == "" {
